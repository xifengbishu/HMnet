...
"""
@Author   : MEIT_WGS
@contact  : guosong_wang@hhu.edu.cn
@File name: CRAM-ConvLSTM
"""
...
def get_3D_ATT_ConvLSTM(input,weight,height,channels, RAM_number, pretrained_weights=None,kernel_size=3):
        inputs = input
        x1_channels = 32
        ker = 3
	conv1 = Conv3D(x1_channels*4, 3, activation='relu', padding='same', kernel_initializer='he_normal')(inputs)

        ram1  = SRRAM3D(conv1,channels=x1_channels*4, RAM_number=2)
	cLSM1 = ConvLSTM2D(x1_channels*4, kernel_size=(ker, ker), padding='same', return_sequences=True)(ram1)
        BN1   = BatchNormalization()(cLSM1)

        ram2  = SRRAM3D(BN1,channels=x1_channels*4, RAM_number=2)
	cLSM2 = ConvLSTM2D(x1_channels*4, kernel_size=(ker, ker), padding='same', return_sequences=True)(ram2)
        BN2   = BatchNormalization()(cLSM2)

        ram3  = SRRAM3D(BN2,channels=x1_channels*4, RAM_number=2)
	cLSM3 = ConvLSTM2D(x1_channels*4, kernel_size=(ker, ker), padding='same', return_sequences=True)(ram3)
        BN3   = BatchNormalization()(cLSM3)

        ram4  = SRRAM3D(BN3,channels=x1_channels*4, RAM_number=2)
	cLSM4 = ConvLSTM2D(x1_channels*4, kernel_size=(ker, ker), padding='same', return_sequences=True)(ram4)
        BN4   = BatchNormalization()(cLSM4)

        conv10 = Conv3D(filters=1, kernel_size=(ker, ker, ker), activation='linear', padding='same', data_format='channels_last')(BN3)
	return conv10
...
def SRRAM3D(input,channels, RAM_number,kernel_size=3):
        ...
        # compute attentions
        ca = CA3D(x1, x1_channels, RAM_number)
        sa = SA3D(x1)
        fa = add([ca, sa])
